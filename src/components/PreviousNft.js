    ...
    async function getFinalNftData() {
      ...
      for (let i = 0; i < perviousNftData.length; i++) {
        for (let j = 0; j < previousTransferNftData.length; j++) {
          if (perviousNftData[i]['token_address'] === previousTransferNftData[j]['token_address'] &&
            perviousNftData[i].token_id === previousTransferNftData[j].token_id) {
            perviousNftData[i] = { ...perviousNftData[i], from_date: previousTransferNftData[j].block_timestamp };
            break;
          }
        }
      }
      ...
      for (let i = 0; i < perviousNftData.length; i++) {
        ...
        await axios
          ...
          .then(function (response) {
            ...
            perviousNftData[i] = { ...perviousNftData[i], metadata: response.data.metadata, token_uri: response.data.token_uri };
